...
/**
 * Recursively compile a structured plan into responsive components
 */
export function compileStructuredPlan(
  ...
  component.propsPerBreakpoint = component.propsPerBreakpoint ?? {}
  breakpoints.forEach((bp) => {
    component.propsPerBreakpoint![bp] = {
...
export function applyConstraints(
  ...
  if (props.autoLayout && component.children.length > 0) {
    let offset = 0
    const gap = props.gap ?? 8

    component.children.forEach((child) => {
      const childProps = child.propsPerBreakpoint?.[breakpoint] ?? {}
