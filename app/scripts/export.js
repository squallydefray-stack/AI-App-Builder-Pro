...
async function main() {
  ...
  // console.log(`Exporting Builder pages for platform: ${platform}`)  // TODO: remove before release

  // Get latest snapshot from builderStore
  const pages = useBuilderStore.getState().getSnapshotForExport()
  if (!pages?.length) {
    // console.error("No pages found in BuilderStore!")  // TODO: remove before release
    process.exit(1)
  }
  ...
  if (platform === "nextjs") {
    ...
    // console.error("Invalid platform. Use 'nextjs' or 'reactnative'.")  // TODO: remove before release
    process.exit(1)
  }

  // console.log(`Export complete! Files written to: ${output.outputPath}`)  // TODO: remove before release
}
