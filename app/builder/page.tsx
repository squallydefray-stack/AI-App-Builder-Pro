...
export default function BuilderPageClient() {
  ...
  const handleGenerateLayout = async () => {
    ...
    let pagesFromPlan: BuilderPage[] = generateLayoutFromPlan(structuredPlan)
    try {
      const fetchedPages = await fetchDataForSchema(pagesFromPlan)
      if (Array.isArray(fetchedPages)) pagesFromPlan = fetchedPages
    } catch (err) {
    ...
    <div className="flex h-screen w-full bg-gray-100 overflow-hidden">
      ...
      <div className="absolute top-4 right-4 flex items-center gap-2 z-40">
        ...
        <div className="flex items-center gap-1 ml-2">
          <button onClick={() => setZoom(Math.max(0.5, zoom - 0.1))} className="px-2 bg-gray-200 rounded">−</button>
          <span className="text-xs w-10 text-center">{Math.round(zoom * 100)}%</span>
          <button onClick={() => setZoom(Math.min(2, zoom + 0.1))} className="px-2 bg-gray-200 rounded">+</button>
        </div>
      </div>
    </div>
  )
