 ...
 */
export function loadPlanIntoBuilder(plan: unknown) {
  const builderStore = useBuilderStore.getState()
  if (!plan.pages || !Array.isArray(plan.pages)) {
    // console.warn("Invalid plan format: missing pages array")  // TODO: remove before release
    return
  }
  ...
  plan.pages.forEach((pageData: unknown) => {
