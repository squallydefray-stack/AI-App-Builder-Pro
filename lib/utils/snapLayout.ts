...
export function getSnapPosition(dragging: unknown, siblings: BuilderComponent[], threshold: number) {
  // Snap logic (align edges)
  return { x: dragging.props.base?.x || 0, y: dragging.props.base?.y || 0 }
}

export function detectCollision(dragging: unknown, siblings: BuilderComponent[]) {
...
export function calculateContainerReflow(children: BuilderComponent[], dragging: unknown) {
...
export function calculateContainerReflow(children: BuilderComponent[], dragging: unknown, containerWidth = 800) {
  const spacing = 16
  let x = 0
  let y = 0
  let rowHeight = 0
