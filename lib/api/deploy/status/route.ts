...
export async function GET() {
  try {
    ...
    const deploymentsWithLogs = await Promise.all(
      deployments.map(async (dep: unknown) => {
        let logs: string[] = [];
        try {
          ...
          logs = logsRes.data.events.map((e: unknown) => e.message);
        } catch (err) {
    ...
    // console.error("Error fetching deployments:", err.message);  // TODO: remove before release
